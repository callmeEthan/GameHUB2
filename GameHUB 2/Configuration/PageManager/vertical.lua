...
function write_data()
	...
	elseif action=='add_entry' then
		local root=SKIN:GetVariable('@')
		if file_exists(root..'User\\page_'..argument..'.inc') then
			argument = ''
			edit = 0
			action = ''
			SKIN:Bang('!SetOption','Message2','Text','File layout already exists')
			SKIN:Bang('!ShowMeter','Message2')
		return end
		SKIN:Bang('[!WriteKeyValue Variables edit_entry "'..edit..'" "#ROOTCONFIGPATH#Configuration\\PageManager\\config2.inc"]')
		SKIN:Bang('[!WriteKeyValue Variables edit_layout "'..argument..'" "#ROOTCONFIGPATH#Configuration\\PageManager\\config2.inc"]')
		SKIN:Bang('[!ActivateConfig "#ROOTCONFIG#\\Configuration\\PageManager" "List2.ini"]')
	end
	argument = ''
	edit = 0
	action = ''
